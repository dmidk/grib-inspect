...
```bash
uv run grib-inspect scan fc2026081200+003_sf.grib2 --db cy43h.sqlite --model cy43h
```
...
```bash
uv run grib-inspect scan fc2026081200+003_ml.grib2 --db cy43h.sqlite --model cy43h
uv run grib-inspect scan fc2026081200+003_pl.grib2 --db cy43h.sqlite --model cy43h
```

Scan the other side:

```bash
uv run grib-inspect scan fc2026081200+003.grib2 --db cy46h.sqlite --model cy46h
```
...
```bash
uv run grib-inspect scan other_run.grib2 --db combined.sqlite --model cy46h
...
uv run grib-inspect scan file.grib2 --db report.sqlite --model x \
...
uv run grib-inspect scan file_sf.grib2 --db report.sqlite --model cy43h --tag part=sf
```
